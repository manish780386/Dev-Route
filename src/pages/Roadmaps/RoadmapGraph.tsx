  ...
  return (
    <div
      ...
      style={{
        ...
        border: `2px solid ${
          data.completed ? "#10b981" : colors.border
        }`,
        borderRadius: 16,
        padding: "12px 16px",
        minWidth: 180,
...
function buildGraphElements(
  ...
  steps.forEach((step) => {
    step.nextSteps.forEach((next) => {
      if (!childrenMap.has(step.id)) {
        childrenMap.set(step.id, []);
      }

      childrenMap.get(step.id)!.push(next);
    });
  });
  ...
  const hasIncoming = new Set(
    steps.flatMap((s) => s.nextSteps)
  );

  const roots = steps.filter(
    (s) => !hasIncoming.has(s.id)
  );
  ...
  while (queue.length > 0) {
    ...
    const { id, level, xOffset } = current;

    if (positioned.has(id)) continue;

    const levelWidth =
      levelWidths.get(level) ?? 0;

    const x = xOffset || levelWidth * SPACING_X;
  ...
  // Auto sequential edges
  const stepsWithNoNext = steps.filter(
    (step) =>
      step.nextSteps.length === 0 &&
      steps.indexOf(step) < steps.length - 1
  );

  stepsWithNoNext.forEach((step) => {
    const nextIndex =
...
export default function RoadmapGraph() {
  const { roadmapId } =
    useParams<{ roadmapId: string }>();

  const roadmap =
    roadmapsData.roadmaps.find(
      (r) => r.id === roadmapId
    );

  const isStepComplete = useStore(
    (state) => state.isStepComplete
  );

  const getProgressPercent = useStore(
    (state) => state.getProgressPercent
  );
  ...
  const completedIds = roadmap.steps
    .filter((step) =>
      isStepComplete(roadmap.id, step.id)
    )
    .map((step) => step.id);
  ...
  const required = roadmap.steps.filter(
    (step) => step.status === "required"
  ).length;
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="bg-amber-50 dark:bg-amber-950 border-b border-amber-100 dark:border-amber-900 px-6 py-2 text-xs text-amber-700 dark:text-amber-400 flex items-center gap-2 shrink-0">
        ...
        <span>
          ...
          to mark steps complete. Scroll and pinch
          to zoom the graph. {required} required
          steps · {roadmap.steps.length} total
        </span>
      </div>
